...
#define MY_PRIVATE_CONFIG "arduino_secrets.h"
...
/* #define WOLFSSL_SERVER_EXAMPLE */
    ...
    char* ssid PROGMEM = MY_SSID;
    char* password PROGMEM  = MY_PASS;
#else
...
/* optional board-specific networking includes */
    ...
    // #error "merde..."
    #include <WiFi.h>
    // #include <WiFiUdp.h> seems to be for upd!
    #ifdef USE_NTP_LIB
        WiFiUDP ntpUDP;
    #endif
...
// JFC crap stuff
void *user_malloc(unsigned long n) { return malloc(n); }
void user_free(void *ptr) { std:free(ptr); }
time_t user_xtime(time_t * timer) { Serial.println("user_xtime"); return 0; } // not OK for the moment ...
...
int setup_certificates(void) {
    ...
    ret = wolfSSL_CTX_use_certificate_buffer(sslctx,
                                             CTX_CLIENT_CERT,
    ...
    */
    ...
    // Serial.println((char *) MYCTX_CA_CERT);
    // Serial.println(MYCTX_CA_CERT_SIZE);
    wolfSSL_Debugging_ON();
    ret = wolfSSL_CTX_load_verify_buffer(sslctx,
                                         MYCTX_CA_CERT,
                                         MYCTX_CA_CERT_SIZE,
                                         MYCTX_CA_CERT_TYPE);
    if (ret == WOLFSSL_SUCCESS) {
...
void setup(void) {
    ...
    wolfSSL_SetIOSend(sslctx, EthernetSend);
    wolfSSL_SetIORecv(sslctx, EthernetReceive);
...
void loop() {
    char reply[1024];
    char msg[]       = "GET /webdav/temp.txt HTTP/1.1\r\nHost: jfclere.myddns.me\r\nUser-Agent: jfctest/0.0.0\r\nAccept: */*\r\n\r\n";
